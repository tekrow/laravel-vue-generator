        ...
        >
            ...
            <template #footer>
                <Button
                    ...
                    @click="
                        $inertia.visit(
                            route('lvg.g-panel.lvg-schematics.create')
                        )
                    "
                    class="p-button-text"
                />
            ...
            <template #footer>
                <Button
                    ...
                    @click="
                        $inertia.visit(
                            route(
                                'lvg.g-panel.lvg-schematics.show',
                                currentModel
                            )
                        )
                    "
            ...
            <template #footer>
                <Button
                    ...
                    @click="
                        $inertia.visit(
                            route(
                                'lvg.g-panel.lvg-schematics.edit',
                                currentModel
                            )
                        )
                    "
...
const apiUrl = route("api.v1.lvg-schematics.dt");
...
const fetchModel = async (row) => {
    axios
        .get(route("api.v1.lvg-schematics.show", row))
        .then((res) => {
...
const deleteModel = async function (row) {
    try {
        const res = await axios.delete(
            route("api.v1.lvg-schematics.destroy", row as any)
        );
        toast.add({
            severity: "success",
