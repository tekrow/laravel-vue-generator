...
const apiUrl = route('api.lvg.schematics.dt');
...
const makeOptionsMenu = (row) => [
    {
        label: "Manage",
        icon: "pi pi-pencil",
        command: () => Inertia.get(route('lvg.g-panel.lvg-schematics.edit', row)),
        visible: () => true,
    },
    {
        label: "Delete",
...
const deleteModel = async function (row) {
    try {
        const res = await axios.delete(route('api.lvg.schematics.destroy', row as any));
