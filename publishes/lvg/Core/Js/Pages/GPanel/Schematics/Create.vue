...
import {Inertia} from "@inertiajs/inertia";
    ...
    const searchTable = async (event) => {
        axios.get(route('api.lvg.g-panel.tables.search', {q: event.query.trim()})).then(r => {
            existingTables.value = r.data
        })
    }
    const createSchematic = async () => {
        form.post(route('lvg.g-panel.lvg-schematics.store'),{
            onSuccess: (res) => {
            ...
            onFinish: (res) => {
                ...
                if (payload) {
                    setTimeout(() => {
                        Inertia.visit(route('lvg.g-panel.lvg-schematics.edit',payload.id));
                    },500)
                }
            }
        })
