<template>
    ...
    <Backend>
        ...
        <div class="border-t p-2 px-3 mt-4 mx-2">
            <Card>
                <template #content>
                    <OrderList v-model="$page.props.lvg.sidebar_menu" dataKey="id">
                        <template #item="{item}">
                                ...
                                <template #header>
                                    <div class="flex items-center justify-between w-full">
                                        ...
                                        <div class="flex gap-x-2 items-center">
                                            <Button @click="$inertia.visit(route('lvg.backend.lvg-menu.edit',item.id))" label="Edit" icon="pi pi-pencil"/>
                                        </div>
                                    </div>
                                </template>
                                        ...
                                        <Panel :collapsed="false" :toggleable="child.item.has_children">
                                            <template #header>
                                                <div class="flex items-center justify-between w-full">
                                                    ...
                                                    <div class="flex gap-x-2 items-center">
                                                        <Button @click="$inertia.visit(route('lvg.backend.lvg-menu.edit',child.item.id))" label="Edit" icon="pi pi-pencil"/>
                                                    </div>
                                                </div>
                                            </template>
                                        </Panel>
