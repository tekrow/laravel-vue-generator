...
if (!function_exists("prepare_menu")) {
    ...
    {
        $devModules = config('lvg.dev_modules',[]);
        return $menu->reject(function (LvgMenu $item) use ($devModules) {
...
if (!function_exists("getGlobalCan")) {
    function getGlobalCan(): array
    {
        ...
        $models = $names->map(fn($name) => ["namespace" => "Lvg\\$name\\Models\\".Str::singular($name),"name" => Str::singular($name)]);
...
if (!function_exists('getSpecialModules')) {
    ...
    {
        return config('lvg.special_modules', []);
    }
}
...
if (!function_exists('getFinishedModules')) {
    ...
    {
        return config('lvg.finished_modules', []);
    }
}
