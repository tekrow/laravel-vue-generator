...
use Lvg\Core\Constants\FormFields;
...
use Tekrow\Lvg\Contracts\ActivatorInterface;
...
class ModuleGenerator extends Generator
{
    ...
    public function generate() : int
    {
        ...
        shell_exec("cd lvg && npx prettier --write $name/");
    ...
    public function generateResources()
    {
        ...
        if (GenerateConfigReader::read('seeder')->generate() === true) {
            $this->console->call('lvg:make-seed', [
                'name' => $this->getPluralName(),
        ...
        if (GenerateConfigReader::read('provider')->generate() === true) {
            $this->console->call('lvg:make-provider', [
                'name' => $this->getPluralName() . 'ServiceProvider',
                'module' => $this->getPluralName(),
                '--master' => true,
            ]);
            $this->console->call('lvg:route-provider', [
                'module' => $this->getPluralName(),
            ]);
        }
...
//            $options = ['--schematic'=> $this->schematic];
            $options = [];
            $this->console->call('lvg:make-factory', [
        ...
        if (GenerateConfigReader::read('model')->generate() === true) {
            $options = ['--schematic'=> $this->schematic];
            $this->console->call('lvg:make-model', [
        ...
        if (GenerateConfigReader::read('policies')->generate() === true) {
            $this->console->call('lvg:make-policy', [
        ...
        if (GenerateConfigReader::read('repository')->generate() === true) {
            $this->console->call('lvg:make-repository', [
                'name' => $this->getPluralName(),
                'module' => $this->getPluralName(),
                '--schematic' => $this->schematic,
            ]);
        ...
        if (GenerateConfigReader::read('request')->generate() === true) {
            ...
            $this->console->call('lvg:make-request', [
        ...
        if (GenerateConfigReader::read('controller')->generate() === true) {
            ...
            $this->console->call('lvg:make-controller', [
        ...
        if ($yes) {
            $this->console->call('lvg:seed',['module' => $this->getPluralName()]);
        }
    }
    ...
    public function makeMenuEntry() {
        ...
        $baseRoute = "lvg.backend.".$this->getLowerNameReplacement();
        if (in_array($this->getLowerNameReplacement(),$auth)) {
            $baseRoute = "lvg.auth.".$this->getLowerNameReplacement();
        }
        $exists = LvgMenu::query()->where("route","=","$baseRoute.index")->exists();
        if (!$exists) {
            $menu = new LvgMenu([
    ...
    public function deleteMenuEntry() {
        $route = "lvg.backend.".$this->getLowerNameReplacement().".index";
        $auth = ['users','roles','permissions'];
        if (in_array($this->getLowerNameReplacement(),$auth)) {
            "lvg.auth.".$this->getLowerNameReplacement().".index";
        }
