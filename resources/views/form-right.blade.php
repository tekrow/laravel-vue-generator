<div class="card">
    <div class="card-header">
        <i class="fa fa-check"></i>{{'{{'}} trans('tekrow/admin-ui::admin.forms.publish') }}
    </div>
...
<div class="card">
    <div class="card-header">
        <i class="fa fa-history"></i> @{{ trans('tekrow/admin-ui::admin.forms.history') }}
    </div>
    <div class="card-body">
        @if(in_array("created_by_admin_user_id", array_column($columns->toArray(), 'name')))

        <div class="form-group row align-items-center">
            <label for="author_id" class="col-form-label text-right col-md-4 col-lg-3">{{'{{'}} trans('tekrow/admin-ui::admin.forms.created_by') }} :</label>
            <user-detail-tooltip :user="form.created_by_admin_user" :edit="true" :datetime="form.created_at" v-if="form.created_by_admin_user">
                <p>{{'{{'}} trans('tekrow/admin-ui::admin.forms.created_on') }} {{'@'}}{{'{{'}} form.created_at  }}</p>
            </user-detail-tooltip>
        </div>
        @endif
        ...
        <div class="form-group row align-items-center">
            <label for="author_id" class="col-form-label text-right col-md-4 col-lg-3">{{'{{'}} trans('tekrow/admin-ui::admin.forms.updated_by') }} :</label>
            <user-detail-tooltip :user="form.updated_by_admin_user" :edit="true" :datetime="form.updated_at" v-if="form.updated_by_admin_user">
                <p>{{'{{'}} trans('tekrow/admin-ui::admin.forms.updated_on') }} {{'@'}}{{'{{'}} form.updated_at }}</p>
            </user-detail-tooltip>
        </div>
        @endif
