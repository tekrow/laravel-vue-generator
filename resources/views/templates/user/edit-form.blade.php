...
@endphp
<template>
    <lvg-tabs :class="`border-none`" nav-classes="bg-secondary-300 rounded-t-lg border-b-4 border-primary">
        <template #nav>
            <lvg-tab-link @activate="activeTab=$event" :active-classes="tabActiveClasses" :tab-controller="activeTab"
        ...
        <template #content>
            <lvg-tab name="basic-info" :tab-controller="activeTab">
...
@elseif($col['type'] === 'text')@php $hasTextArea = true; echo "\r"; @endphp
                    <div class=" sm:col-span-4">
                        <jet-label for="{{$col['name']}}" value="{{$col['label']}}" />
                        <lvg-textarea class="w-full" id="{{$col['name']}}" name="{{$col['name']}}" v-model="form.{{$col['name']}}"
                                      :class="{'border-red-500 sm:focus:border-red-300 sm:focus:ring-red-100': form.errors.{{$col['name']}}}"
                        ></lvg-textarea>
                        <jet-input-error :message="form.errors.{{$col['name']}}" class="mt-2" />
                    </div>
...
@endif
            ...
            <lvg-tab name="assign-roles" :tab-controller="activeTab">
...
<script>
    ...
    import LvgTab from "@/LvgComponents/LvgTab.vue";
    import LvgTabs from "@/LvgComponents/LvgTabs.vue";
    import LvgTabLink from "@/LvgComponents/LvgTabLink.vue";

    @if($hasCheckbox)import JetCheckbox from "@/Jetstream/Checkbox.vue";
@endif
@if($hasDate)import LvgDatepicker from "@/LvgComponents/LvgDatepicker.vue";
@endif
    @if($hasInput)import JetInput from "@/Jetstream/Input.vue";
@endif
    @if($hasTextArea)import LvgTextarea from "@/LvgComponents/LvgTextarea.vue";
@endif
    @if($hasSelect)import InfiniteSelect from '@/LvgComponents/InfiniteSelect.vue';
@endif
...
@endif
            @if($hasDate)LvgDatepicker,
@endif
            @if($hasCheckbox)JetCheckbox,
@endif
            @if($hasTextArea)LvgTextarea,
@endif
